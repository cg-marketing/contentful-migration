...
import { getConfig, CasinoClientConfig } from "./lib/config";
...
const createRun = ({ shouldThrow }) =>
  async function runSingle(argv) {
    ...
    const application =
      argv.managementApplication || `contentful.migration-cli/${version}`;
    ...
    let thisMigrationHistory = history
      .filter((m) => m.migrationName === migrationName && m.completed)
      .pop();
    if (thisMigrationHistory) {
      console.error(
        chalk`{gray Migration previously completed at ${new Date(
          thisMigrationHistory.completed
        ).toString()}}`
      );
      if (!argv.force) {
        return;
      ...
      console.error(
        chalk`  {gray Re-running migration anyways due to "--force" parameter}`
      );
    } else {
      thisMigrationHistory = history
        .filter((m) => m.migrationName === migrationName)
        .pop();
      if (thisMigrationHistory) {
        console.error(
          chalk`⚠️  {bold.yellow Migration failed before completion at ${new Date(
            thisMigrationHistory.started
          ).toString()}}`
        );
        if (!argv.force) {
          console.error(
            chalk`  {bold.yellow Please manually inspect the data model for errors and then re-run using "--force"}`
          );
          return;
        }
        console.error(
...
const createRunBatch = ({ shouldThrow }) =>
  async function runBatch(argv) {
    ...
    migrationFunctions.push(
      ...files.map((f) => loadMigrationFunction(path.resolve(process.cwd(), f)))
    );
        ...
        if (thisMigrationHistory) {
          console.error(
            chalk`{gray Migration ${(
              i + 1
            ).toString()}: ${migrationName}\n  previously completed at ${new Date(
              thisMigrationHistory.completed
            ).toString()}}`
          );
        } else {
          ...
          if (thisMigrationHistory) {
            console.error(
              chalk`⚠️  {bold.yellow Migration ${(
                i + 1
              ).toString()}: ${migrationName}\n  failed before completion at ${new Date(
                thisMigrationHistory.started
              ).toString()}}`
            );
            if (!argv.force) {
            ...
            console.error(
              chalk`  {bold.yellow   Re-running migration anyways due to "--force" parameter}`
            );
          }
          console.error(
            chalk`{bold.cyan Migration ${(i + 1).toString()}: ${migrationName}}`
          );
...
async function execMigration(
  ...
  if (parseResult.hasStepsValidationErrors()) {
    ...
    terminate(
      new ManyError("Step Validation Errors", parseResult.stepsValidationErrors)
    );
  }
  ...
  if (parseResult.hasPayloadValidationErrors()) {
    ...
    terminate(
      new ManyError(
        ...
        parseResult.payloadValidationErrors
      )
    );
  }
  ...
  if (parseResult.hasValidationErrors()) {
    ...
    terminate(
      new ManyError("Validation Errors", parseResult.getValidationErrors())
    );
  }
  ...
  if (config.persistToSpace) {
    tasks.splice(0, 0, {
      ...
      task: async () => {
        ...
        const resp = await environment.createEntry(
          "migrationHistory",
          thisMigrationHistory.update({})
        );
  ...
  if (answers.applyMigration) {
    try {
      ...
      console.error(
        `Please check the errors log for more details: ${errorsFile}`
      );
      terminate(err);
    }
  } else {
...
function loadMigrationFunction(filePath) {
  try {
    ...
    console.error(
      chalk`{red.bold The ${filePath} script could not be parsed, as it seems to contain syntax errors.}\n`
    );
    console.error(e);
    process.exit(1);
  }
...
interface IRunConfig extends CasinoClientConfig {
  accessToken?: string;
  spaceId?: string;
