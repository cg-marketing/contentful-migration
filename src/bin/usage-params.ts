...
export default yargs
  ...
  .command(
    "batch <directory|glob>",
    "Run a set of migration scripts in filename order",
    {},
    runBatchMigration
  )
  .coerce("filePath", (filePath) => {
    ...
    if (fs.statSync(filePath).isDirectory()) {
      throw new Error(
        chalk`{bold.red Cannot load migration file ${filePath}: is a directory.\n  Did you mean to run in 'batch' mode?}`
      );
    }
    return filePath;
  })
  ...
  .option("force", {
    boolean: true,
    describe:
      "Re-runs any migrations that previously errored or have already completed.",
    default: false,
  })
  .option("persist-to-space", {
    ...
    boolean: true,
    describe:
      "Persists the fact that this migration ran in a History content-type in the Contentful space",
    default: false,
  })
